...
{{--	@dd($appointment)--}}
	@if(auth ()->user ()->acc_type=='Doctor')
		<form action="/create/invoice/{{ $appointment->id }}/clear">
			<button
		...
		<form action="/create/invoice/{{ $appointment->id }}/generate/{{ auth()->user ()->name }}" method="POST" enctype="multipart/form-data">
			@csrf
			<label>Date: </label>
...
{{--								{{ auth ()->user ()->acc_type == 'admin' ? '' : 'disabled' }}--}}
								disabled
				...
				<label for="patient_name"
							 class="block mb-2 text-sm font-medium text-gray-900 pt-4 mr-5">Patient:</label>
				<select id="patient_name" name="patient_name" disabled
								class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-1/6 p-2.5 mt-2 mr-5">
...
{{--					@endforeach--}}
							<option
								value="{{ $appointment->id }}">{{ $appointment->firstName . ' ' . $appointment->lastName }}</option>
				</select>
		...
		<div>
			...
			<form action="/create/invoice/{{ $appointment->id }}/add">
				...
				<div class="form-outline mt-5">
					<input type="number" name="qty" required id="typeNumber"
								 class="border border-gray-300 rounded-lg w-1/6 form-control mb-4" placeholder="Enter Qty"
								 min="1"
								 max="100"/>
				</div>
				<button
