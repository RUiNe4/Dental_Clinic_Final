        ...
        @if( count($invoice_items) > 0)
            <ul class="flex flex-col">
                @for($i = 0; $i < count($invoice_items);$i++)
                    <li class="border-l-4 border-[#4F9298] shadow-md bg-white my-2 px-8 py-4"
                        x-data="accordion({{ $invoice_items[$i][0]->id }})">
                        <h2 class="flex flex-row justify-between items-center font-semibold cursor-pointer"
                            @click="handleClick()">
...
{{--										 {{ $invoice_items[$i][0]->invoice_id }}--}}
										...
										<span class="text-xs font-bold ">{{ $invoices[$i]->date }} </span>
									</div>
								<div>
									<span class="font-normal text-sm">Total: </span> <span
                                        class="text-sm">${{ $invoices[$i]->amount }}</span>
								</div>
							</span>
