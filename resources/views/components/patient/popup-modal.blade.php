            ...
            <li class="bg-white my-2" x-data="accordion({{ $patient->id }})">
                <h2 class="flex flex-row justify-between items-center font-semibold p-3 cursor-pointer"
                    ...
                    <span style="color:#4F9298">
                        <a href="{{ request()->path() == 'doctor/mailbox' ? url('/doctor/mailbox/' . $patient->id) : url('/appointment/' . $patient->id) }}"
                            class="font-bold">
                        ...
                        <div class="text-sm">
                            ...
                            <a href="{{ $patient->paid ? '#' : url('/create/invoice') }}">
                                <span class="text-xs font-bold ">{{ $patient->paid ? 'PAID' : 'PENDING' }} </span>
                            </a>
                        </div>
                    </span>
                    <svg :class="handleRotate()" fill="#4F9298"
                        class="text-purple-700 h-6 w-6 transform transition-transform duration-500" viewBox="0 0 20 20">
                        <path
                ...
                <div x-ref="tab" :style="handleToggle()"
                    ...
                    <form action="/appointment/{{ $patient->id }}/change" wire:submit.prevent="">
                        ...
                        <div class="flex items-center p-6 space-x-2 border-gray-200 rounded-b dark:border-gray-600">
                            ...
                            @else
                                <button data-modal-hide="staticModal" type="submit" name="res" value="paid"
                                    class="text-white bg-amber-700 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5  focus:z-10" disabled>
                                    PAID
                                </button>
                            @endif
