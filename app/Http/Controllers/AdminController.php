...
' );
        ...
        public function updateTreatment ( Treatment $treatment , Request $request )
        {
            ...
            $treatment -> update ();
            return redirect () -> back () -> withErrors ( [ 'price' => 'Update Successfully' ] );
        }
...
' );
        ...
        {
            $patients = Appointment ::where ( 'appointedDoctor' , $user -> name ) -> get ();
            foreach ( $patients as $patient ) {
                $patient -> appointedDoctor = NULL;
                if ( $patient -> paid == 1 ) {
                    $patient -> status = 'Approve';
                } else {
        ...
        public function updatePassword ( Request $request , User $user )
        {
            ...
            if ( self ::passwordCorrect ( $request[ 'oldPassword' ] , $user -> password ) ) {
                if ( $request[ 'password' ] != $request[ 'password_confirmation' ] ) {
                    return back () -> withErrors ( [ 'password' => 'Passwords do not match' ] ) -> onlyInput ( 'password' );
                }
        ...
        {
            $appointments = Appointment ::where ( 'appointedDoctor' , $user -> name ) -> get ();
            foreach ( $appointments as $appointment ) {
                $appointment -> appointedDoctor = $request[ 'name' ];
                $appointment -> update ();
            }
            $invoices = Invoice ::where ( 'doctor' , $user -> name ) -> get ();
